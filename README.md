...
| Claude 호환용 skill 경로 | optional `.codex/skills/*` compatibility mirror |
| `.claude/agents/*.md` | `.codex/agents/*.md` role card |
| hooks | 명시적 validation + `namba sync` + structured logs |
| custom slash command workflow | `$namba` skill(예: `$namba-run` 트리거 포함) + built-in Codex slash commands + `namba` CLI |
...
compat_skills_path: .codex/skills  # 비워두면 compatibility mirror 생성 생략
