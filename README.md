...
| `.claude/agents/*.md` | `.codex/agents/*.toml` custom agent + `.md` readable mirror |
| hooks | 명시적 validation + `namba sync` + structured logs |
| custom slash command workflow | `$namba` + `$namba-run` 같은 command-entry skill + built-in Codex slash commands + `namba` CLI |
...
설치된 전역 `namba`는 다음 명령으로 GitHub Release 기준 최신 버전으로 올립니다.
...
- 개발 방법론: `TDD` 또는 `DDD`
- 언어 / 프레임워크
- 작업 언어: Codex 대화 / 문서 / PR / 코드 주석에 공통 적용
...
```powershell
namba init . --yes --name example --mode tdd --agent-mode multi --human-language ko --approval-policy on-request --sandbox-mode workspace-write --git-mode team --git-provider github --git-username alice
```
...
- `$namba`를 직접 호출
- `$namba-run`, `$namba-plan`, `$namba-sync` 같은 command-entry skill 호출
- `namba project`
- `namba regen`
...
```text
namba project
namba regen    # when template-generated Codex assets need regeneration
namba plan "work description"
namba run SPEC-XXX
namba sync
```

`namba update`, `namba regen`, and `namba sync` are different commands.
...
## Update vs Regen vs Sync

- `namba update`: 설치된 `namba` binary를 GitHub Release 기준으로 self-update합니다. `--version vX.Y.Z`로 특정 버전 지정이 가능합니다.
- `namba regen`: `AGENTS.md`, `.agents/skills/`, `.codex/agents/*.toml`, `.codex/agents/*.md`, `.namba/codex/*`, `.codex/config.toml`을 `.namba/config/sections/*.yaml` 기준으로 다시 생성합니다.
- `namba sync`: refresh `.namba/project/*` docs, codemaps, change summary, PR checklist, and release notes/checklists.
- If you edit template-generated assets directly, a later `namba regen` can overwrite them. Durable settings belong in `.namba/config/sections/*.yaml`.

## Command-Entry Skills

- Codex custom slash-command 확장이 아직 제한적이기 때문에, NambaAI는 command-entry skill로 같은 역할을 제공합니다.
- 기본 라우터는 `$namba`이고, 직접 실행형 진입점은 `$namba-init`, `$namba-project`, `$namba-regen`, `$namba-update`, `$namba-plan`, `$namba-fix`, `$namba-run`, `$namba-sync`입니다.
...
- `AGENTS.md`는 NambaAI의 보고형 응답 계약을 정의합니다. 큰 응답은 `NAMBA-AI 작업 결과 보고` 같은 헤더 아래에서 `작업 정의 -> 판단 -> 수행한 작업 -> 현재 이슈 -> 잠재 문제 -> 다음 스텝` 순서의 보고 구조를 따릅니다.
- 출력이 단조롭지 않도록 의미 순서는 고정하지만, `namba init`에서 고른 언어에 맞춰 헤더와 섹션 라벨이 바뀌고 간단한 이모지 마커도 함께 사용할 수 있습니다.
...
- SPEC 작업은 `spec/<SPEC-ID>-<slug>`, 일반 작업은 `task/<slug>` 형식의 브랜치 이름을 권장합니다.
- 커밋은 작업 브랜치에 쌓고, PR base branch는 항상 `main`으로 유지합니다.
- PR 제목, 본문, 리뷰 포인트는 한글로 작성합니다.
- PR이 열리면 GitHub에서 `@codex review`를 요청합니다.
- 이 저장소는 `.github/workflows/codex-review-request.yml`로 `main` 대상 PR에 `@codex review` 댓글을 자동 등록합니다. 실제 리뷰 연결에는 ChatGPT Codex Connector GitHub App 설치가 필요합니다.
...
대신 Codex에서 실제로 로드할 **custom agent TOML**을 생성하고, 읽기 쉬운 `.md` role card를 함께 둡니다.
...
`.toml` 파일은 Codex custom agent 정의이고, `.md` 파일은 사람이 읽기 쉬운 mirror입니다.
...
- repo-local command-entry skills 지원
- repo-local Codex custom agent 지원
