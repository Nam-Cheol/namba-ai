...
| Claude 호환용 skill 경로 | `.codex/skills/*` compatibility mirror (placeholder files to avoid duplicate skill registration) |
| `.claude/agents/*.md` | `.codex/agents/*.md` role card |
| hooks | 명시적 validation + `namba sync` + structured logs |
| custom slash command workflow | `$namba` / `$namba-run` skills + built-in Codex slash commands + `namba` CLI |
