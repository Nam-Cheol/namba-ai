...
func renderManagedProjectRoot(lang string, projectCfg projectConfig, profile initProfile, cfg docsConfig) string {
	...
	case "ko":
		return strings.Join([]string{
			...
			"- `namba plan` 또는 `namba fix`로 요청을 SPEC 작업 패키지로 바꿉니다.",
	...
	case "ja":
		return strings.Join([]string{
			...
			"- `namba plan` または `namba fix` で依頼を SPEC 作業パッケージに変換します。",
	...
	case "zh":
		return strings.Join([]string{
			...
			"- 用 `namba plan` 或 `namba fix` 把需求转成 SPEC 工作包。",
