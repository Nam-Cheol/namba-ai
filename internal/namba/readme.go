...
func renderNambaCLIGettingStartedBootstrapSection(lang string) []string {
	...
	case "ko":
		return []string{
			...
			"- 첫 interactive Codex 세션에서 `6 hooks need review`가 보이면 `/hooks`를 열고 `.codex/hooks/namba_codex_guard.py` 명령을 확인한 뒤 승인하세요. 이 승인이 끝나야 애매한 prompt를 질문 쪽으로 유도하는 hook이 실행됩니다.",
...
func renderNambaCLIRootHookRuntimeSection(lang string) []string {
	...
	case "ko":
		return []string{
			...
			"- Codex lifecycle hook은 `namba init .`가 `.codex/hooks.json`과 `.codex/hooks/namba_codex_guard.py`로 미리 생성합니다. 이 hook은 Namba context를 주입하고, 애매한 prompt는 전송을 막지 않고 질문/정리 쪽으로 유도하며, 권한 요청의 위험성을 설명하고, final response 형식과 managed instruction surface 변경 후 `namba regen`/validation을 상기시킵니다.",
