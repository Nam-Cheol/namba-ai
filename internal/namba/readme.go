...
func renderNambaCLIGettingStartedBootstrapSection(lang string) []string {
	...
	default:
		return []string{
			...
			"- In the first interactive Codex session, open `/hooks` when you see `6 hooks need review`, confirm the `.codex/hooks/namba_codex_guard.py` commands, then approve them. Prompt-refinement hooks will not run until this review is complete.",
...
func renderNambaCLIRootHookRuntimeSection(lang string) []string {
	...
	case "ja":
		return []string{
			...
			"- Codex lifecycle hook は `namba init .` が `.codex/hooks.json` と `.codex/hooks/namba_codex_guard.py` として事前生成します。この hook は Namba context を追加し、曖昧な prompt は先にブロックして質問と整理を要求し、権限要求のリスクを説明し、final response 形式と managed instruction surface 変更後の `namba regen` / validation を思い出させます。",
...
func renderManagedProjectRootWorkSummarySection(lang string) []string {
	...
	default:
		return []string{
			...
			"- `frontend-major` briefs include a Do-Not Design Contract, and run results must report a Do-Not Design Violation Check before completion.",
...
func renderManagedProjectRootCustomAgentsSection(lang string) []string {
	...
	default:
		return []string{
			...
			"- For `frontend-major`, `namba-designer` owns the Do-Not Design Contract, `namba-frontend-architect` consumes the architecture handoff, and `namba-frontend-implementer` reports Do-Not Design Violation Check evidence.",
			"- Routing examples: `Redesign this landing page hero so it stops looking generic` -> `namba-designer`; `Plan the component/state split for this dashboard` -> `namba-frontend-architect`; `Implement the approved dashboard filters and responsive states` -> `namba-frontend-implementer`.",
...
func renderNambaCLIWorkflowGuide(lang string) string {
	...
	default:
		...
		lines = append(lines,
			...
			"- Explicit `frontend-major` runs block when the Do-Not Design Contract is missing or insufficient, and implementation output must include a Do-Not Design Violation Check.",
			...
			"- For `frontend-major`, `namba-designer` owns the Do-Not Design Contract, `namba-frontend-architect` consumes the architecture handoff, and `namba-frontend-implementer` reports Do-Not Design Violation Check evidence against the approved visual grammar.",
			...
			"- Frontend-touching planning writes `.namba/specs/<SPEC>/frontend-brief.md`; explicit `frontend-major` briefs include a Do-Not Design Contract with banned generic fallbacks, allowed replacements, visual grammar, architecture handoff, and violation checks.",
...
func renderManagedProjectGettingStartedOpenSection(lang, projectName string) []string {
	...
	case "ko":
		return []string{
			...
			"- `6 hooks need review`가 보이면 `/hooks`를 열고 `.codex/hooks/namba_codex_guard.py` 명령을 확인한 뒤 승인하세요. 그래야 NambaAI의 prompt 교정 hook이 실행됩니다.",
	...
	case "ja":
		return []string{
			...
			"- `6 hooks need review` が表示されたら `/hooks` を開き、`.codex/hooks/namba_codex_guard.py` の command を確認してから承認してください。これで NambaAI の prompt refinement hook が実行されます。",
	...
	case "zh":
		return []string{
			...
			"- 看到 `6 hooks need review` 时，打开 `/hooks`，确认 `.codex/hooks/namba_codex_guard.py` command 后再批准。这样 NambaAI 的 prompt refinement hook 才会运行。",
	...
	default:
		return []string{
			...
			"- When Codex shows `6 hooks need review`, open `/hooks`, confirm the `.codex/hooks/namba_codex_guard.py` commands, then approve them so NambaAI prompt-refinement hooks can run.",
...
func renderManagedProjectWorkflowGuideReviewReadinessSection(lang string) []string {
	...
	default:
		return []string{
			...
			"- `frontend-major` briefs include a Do-Not Design Contract so banned generic fallbacks, allowed replacements, visual grammar, architecture handoff, and post-implementation violation checks are visible before coding.",
			...
			"- Explicit `frontend-major` runs can still block on missing, insufficient, invalid, mismatched, or negative-first contract evidence, and implementation results must include a Do-Not Design Violation Check.",
			"",
		}
	}
}
