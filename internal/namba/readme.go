...
func renderNambaCLIRoot(lang string, cfg docsConfig) string {
	...
	case "ko":
		lines := []string{
			...
			"- `namba project`로 현재 구조와 문서를 새로 고치고, `namba plan`으로 기능 SPEC를, `namba harness`로 agent/skill/workflow 재사용을 위한 harness-oriented SPEC를 계획하고, `namba fix --command plan`으로 review 가능한 버그 수정 SPEC를 만들거나, `namba fix`로 현재 workspace에서 직접 수리를 시작할 수 있습니다.",
		...
		lines = append(lines,
			...
			"- `$namba-harness`: agent, skill, workflow, orchestration 재사용을 위한 harness-oriented SPEC 패키지를 만듭니다.",
