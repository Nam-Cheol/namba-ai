...
func renderNambaCLIRoot(lang string, cfg docsConfig) string {
	...
	case "ko":
		...
		lines = append(lines,
			...
			"- `$namba-create`: repo-local skill, project-scoped custom agent, 또는 둘 다를 preview-first로 만들고 싶을 때 씁니다. 이 경로는 `namba create` 같은 새 CLI를 추가하지 않고 Codex 안에서 동작합니다.",
			...
			"- `$namba-create` -> `.agents/skills/*` 또는 `.codex/agents/*`를 위한 skill-first 생성 흐름, phase 1에서는 새 `namba create` CLI 없음",
...
func renderManagedProjectRoot(lang string, projectCfg projectConfig, profile initProfile, cfg docsConfig) string {
	...
	default:
		return strings.Join([]string{
			...
			"- `$namba-create`: create a repo-local skill, a project-scoped custom agent, or both through the preview-first Codex flow. Phase 1 stays skill-first and does not add a new `namba create` CLI command.",
