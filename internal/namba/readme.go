...
func renderRootCTARow(lang string) []string {
	lines := strings.Split(renderRootCTAButtonRow(lang), "\n")
	return append(lines, "")
}
...
func renderNambaCLIGettingStartedBootstrapSection(lang string) []string {
	...
	default:
		return []string{
			...
			"```",
			"",
			"The wizard starts with a language selection screen. You can choose `[ko] Korean`, `[en] English`, `[ja] Japanese`, or `[zh] Simplified Chinese` by number or code without flag emoji labels; after that, repository state and defaults are explained in the selected language. Existing repositories keep detected stack defaults; empty repositories leave the app stack unset and only prepare the Namba/Codex environment. Plain terminals use `[default]`, `[recommended]`, `[current]`, and `[next]` markers for progress, defaults, recommendations, and next steps. `b` or `back` returns to the previous step for edits. It does not ask for a GitHub username, and the Codex access preset previews the resulting approval_policy / sandbox_mode pair.",
...
func renderNambaCLIRootQualityGatesSection(lang string) []string {
	...
	case "ko":
		return []string{
			...
			"- `go.mod`의 `toolchain go1.26.5` 기준을 사용하므로 CI와 `GOTOOLCHAIN=auto` 로컬 실행이 알려진 Go 1.26.3 표준 라이브러리 취약점을 피합니다.",
	...
	case "zh":
		return []string{
			...
			"- 该仓库使用 `go.mod` 中的 `toolchain go1.26.5`，因此 CI 和 `GOTOOLCHAIN=auto` 的本地运行会避开已知的 Go 1.26.3 标准库漏洞。",
	...
	default:
		return []string{
			...
			"- `go.mod` pins `toolchain go1.26.5`, so CI and local `GOTOOLCHAIN=auto` runs avoid the known Go 1.26.3 standard-library vulnerabilities.",
...
func renderNambaCLIWorkflowGuideCommandDifferencesSection(lang string) []string {
	...
	case "zh":
		return []string{
			...
			"- Codex 运行 `namba update` 前必须先询问用户。",
...
func renderNambaCLIRoot(lang string, cfg docsConfig) string {
	...
	case "zh":
		lines := []string{
			...
			"# NambaAI",
			"",
			"NambaAI 是一份给 Codex 工作流用的操作指南，帮助你少纠结“下一步该做什么”。核心差异点是执行前的 prompt refinement：先把模糊想法整理成目标、范围、约束和验收标准，再让 Codex 开始编码。它会帮你准备仓库、选择当前该用的命令、把较大的工作拆成可评审的计划，并在完成后同步文档和清单。",
			"",
			renderLanguageLinks(""),
			"",
			renderRootLinkBar(),
...
func renderManagedProjectRootWorkSummarySection(lang string) []string {
	...
	case "ko":
		return []string{
			...
			"- `namba plan`은 `게시판 만들어줘`처럼 짧고 모호한 요구를 바로 SPEC로 만들지 않고, Goal/Scope/Constraints/Acceptance가 잡힐 때까지 먼저 질문합니다.",
	...
	case "ja":
		return []string{
			...
			"- `namba plan` は `掲示板を作って` のような短く曖昧な要求をすぐ SPEC にせず、Goal/Scope/Constraints/Acceptance が揃うまで先に質問します。",
	...
	case "zh":
		return []string{
			...
			"- `namba plan` 不会把“做个论坛”这类短而含糊的需求直接变成 SPEC；它会先提问，直到 Goal/Scope/Constraints/Acceptance 足够清楚。",
	...
	default:
		return []string{
			...
			"- Use `namba plan` for feature SPEC work; by default it hands the created SPEC to `$namba-plan-review` unless you pass `--no-review`. Use `namba harness` for harness-oriented SPEC work, `namba fix --command plan` for bugfix SPEC work, or `namba fix` for direct repair in the current workspace.",
			"- `namba plan` does not turn short ambiguous requests such as `make a forum` directly into a SPEC; it asks first until Goal/Scope/Constraints/Acceptance are clear enough.",
...
func renderManagedProjectRootCommandSkillsSection(lang string) []string {
	...
	default:
		return []string{
			...
			"- `$namba-plan`: clarify vague feature requests first, then create the next feature SPEC package and continue with `$namba-plan-review` unless `--no-review` is present.",
...
func renderManagedProjectGettingStartedOpenSection(lang, projectName string) []string {
	...
	case "ko":
		return []string{
			...
			"- `6 hooks need review`가 보이면 `/hooks`를 열고 `.codex/hooks/namba_codex_guard.sh` 또는 Windows `.codex/hooks/namba_codex_guard.ps1` launcher가 `.codex/hooks/namba_codex_guard.py`를 실행하는지 확인한 뒤 승인하세요. 그래야 NambaAI의 prompt 교정 hook이 실행됩니다.",
	...
	case "ja":
		return []string{
			...
			"- `6 hooks need review` が表示されたら `/hooks` を開き、`.codex/hooks/namba_codex_guard.sh` または Windows `.codex/hooks/namba_codex_guard.ps1` launcher が `.codex/hooks/namba_codex_guard.py` を実行することを確認してから承認してください。これで NambaAI の prompt refinement hook が実行されます。",
	...
	case "zh":
		return []string{
			...
			"- 看到 `6 hooks need review` 时，打开 `/hooks`，确认 `.codex/hooks/namba_codex_guard.sh` 或 Windows `.codex/hooks/namba_codex_guard.ps1` launcher 会执行 `.codex/hooks/namba_codex_guard.py` 后再批准。这样 NambaAI 的 prompt refinement hook 才会运行。",
	...
	default:
		return []string{
			...
			"- When Codex shows `6 hooks need review`, open `/hooks`, confirm the `.codex/hooks/namba_codex_guard.sh` or Windows `.codex/hooks/namba_codex_guard.ps1` launcher runs `.codex/hooks/namba_codex_guard.py`, then approve it so NambaAI prompt-refinement hooks can run.",
...
func renderManagedProjectGettingStartedWorkPackageSection(lang string) []string {
	...
	case "ko":
		return []string{
			"## 3. 작업 패키지 만들기",
			"",
			"- Namba runner `codex exec` access 기본값 확인/변경: `namba codex access`",
	...
	case "ja":
		return []string{
			"## 3. 作業パッケージを作る",
			"",
			"- Namba runner `codex exec` access default の確認 / 変更: `namba codex access`",
	...
	case "zh":
		return []string{
			"## 3. 创建工作包",
			"",
			"- 查看或修改 Namba runner `codex exec` access 默认值：`namba codex access`",
	...
	default:
		return []string{
			...
			"",
			"- Inspect or change Namba runner `codex exec` access defaults: `namba codex access`",
...
func renderManagedProjectWorkflowGuidePlanAndFixSection(lang string) []string {
	...
	case "ko":
		return []string{
			...
			"- `namba project`: 현재 저장소 문서와 codemap을 새로 고치며 SPEC 패키지를 만들지 않습니다.",
			"- `namba codex access`: Namba runner의 `codex exec` access 기본값을 확인하고, 명시적 flag를 줄 때만 `.namba/config/sections/system.yaml`의 approval_policy / sandbox_mode를 바꿉니다.",
			"- `namba plan \"description\"`: 다음 기능 SPEC 패키지와 review artifact를 만들고, `--no-review`가 없으면 `$namba-plan-review`로 바로 이어갑니다.",
			"- `namba harness \"description\"`: agent, skill, workflow, orchestration 재사용을 위한 harness-oriented SPEC 패키지와 review artifact를 만듭니다.",
			"- `namba fix --command plan \"issue description\"`: 버그 수정 SPEC 패키지와 review artifact를 만듭니다.",
			"- planning 기본값: 현재 workspace에서 전용 `spec/...` 브랜치를 만들거나 전환해 scaffold합니다. `--current-workspace`는 새 SPEC 브랜치 없이 현재 브랜치에 바로 scaffold하려는 경우에만 쓰고, worktree는 임시 `namba run SPEC-XXX --parallel` 실행에만 사용합니다.",
	...
	case "ja":
		return []string{
			...
			"- `namba project`: 現在のリポジトリ docs と codemap を更新し、SPEC パッケージは作成しません。",
			"- `namba codex access`: Namba runner の `codex exec` access default を確認し、明示的な flag を渡したときだけ `.namba/config/sections/system.yaml` の approval_policy / sandbox_mode を変更します。",
			"- `namba plan \"description\"`: 次の機能 SPEC パッケージと review artifact を作成し、`--no-review` がなければ `$namba-plan-review` に続けます。",
			"- `namba harness \"description\"`: agent / skill / workflow / orchestration 再利用向けの harness-oriented SPEC パッケージと review artifact を作成します。",
			"- `namba fix --command plan \"issue description\"`: バグ修正 SPEC パッケージと review artifact を作成します。",
			"- planning の既定動作: 現在の workspace で専用 `spec/...` branch を作成または checkout して scaffold します。`--current-workspace` は新しい SPEC branch を作らず現在の branch に直接 scaffold したい場合だけ使い、worktree は一時的な `namba run SPEC-XXX --parallel` 実行にだけ使います。",
	...
	case "zh":
		return []string{
			...
			"- `namba project`: 刷新当前仓库文档和 codemap，不会创建 SPEC 包。",
			"- `namba codex access`: 查看 Namba runner 的 `codex exec` access 默认值，只有传入明确 flag 时才会修改 `.namba/config/sections/system.yaml` 中的 approval_policy / sandbox_mode。",
			"- `namba plan \"description\"`: 创建下一个功能 SPEC 包和 review artifact；如果没有 `--no-review`，会继续交给 `$namba-plan-review`。",
			"- `namba harness \"description\"`: 创建面向 agent / skill / workflow / orchestration 复用的 harness-oriented SPEC 包和 review artifact。",
			"- `namba fix --command plan \"issue description\"`: 创建缺陷修复 SPEC 包和 review artifact。",
			"- planning 默认行为：在当前 workspace 中创建或切换到专用 `spec/...` branch 再 scaffold。`--current-workspace` 只用于明确要求直接在当前 branch 上 scaffold、不新建 SPEC branch；worktree 只用于临时的 `namba run SPEC-XXX --parallel` 执行。",
...
func renderManagedProjectWorkflowGuidePlanningCommandsSection() []string {
	return []string{
		...
		"- `namba codex access`: inspect Namba runner `codex exec` access defaults and mutate `.namba/config/sections/system.yaml` only when explicit approval_policy / sandbox_mode flags are present.",
