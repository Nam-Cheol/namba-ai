...
func renderNambaCLIRoot(lang string, cfg docsConfig) string {
	...
	default:
		return strings.Join([]string{
			...
			fmt.Sprintf("- [%s](%s): update vs regen vs sync vs pr vs land, generated assets, and collaboration defaults", localizeGuideLabel(lang, "workflow-guide"), guidePath("workflow-guide", lang)),
...
func renderManagedProjectRoot(lang string, projectCfg projectConfig, profile initProfile, cfg docsConfig) string {
	...
	default:
		return strings.Join([]string{
			...
			"- Execute `namba run SPEC-XXX`, validate changes, refresh docs with `namba sync`, and use `namba pr` plus `namba land` for the GitHub review cycle.",
