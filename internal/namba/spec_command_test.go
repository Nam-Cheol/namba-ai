...
func TestRunFixCommandPlanClarificationRunsBeforeProjectLookup(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "korean", args: []string{"fix", "--command", "plan", "버그", "고쳐줘"}},
		{name: "english", args: []string{"fix", "--command", "plan", "bug"}},
	} {
		tc := tc
...
func TestFixUsageTextMatchesSubcommandBehaviorSummaries(t *testing.T) {
	...
	if thirdExampleIndex >= strings.Index(got, fixSubcommandDefinitions()[0].BehaviorSummary) {
...
func TestRunPlanDoesNotWriteHarnessSidecarForCodexArtifactPlan(t *testing.T) {
	t.Parallel()
	...
	if _, err := os.Stat(filepath.Join(tmp, ".namba", "specs", "SPEC-001", harnessRequestFileName)); !os.IsNotExist(err) {
		t.Fatalf("expected codex artifact plan to avoid harness sidecar, stat err=%v", err)
	}
}
...
func TestBuildSpecPackageScaffoldOutputsAddsFrontendBriefForFrontendMajorWork(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Frontend implementation phase: first-frontend",
		"Asset mode: generated-images",
		"Imagegen requirement: required",
		"Asset decision proof:",
