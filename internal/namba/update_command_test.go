...
func TestRunRegenRegeneratesCodexAssetsFromConfig(t *testing.T) {
	...
	if !strings.Contains(config, "#:schema https://developers.openai.com/codex/config-schema.json") || !strings.Contains(config, "avoids session-owned Codex choices") || !strings.Contains(config, "[features]") || !strings.Contains(config, "hooks = true") || !strings.Contains(config, "goals = true") || !strings.Contains(config, "max_threads = 5") {
	...
	for _, unwanted := range []string{`approval_policy =`, `sandbox_mode =`} {
		if strings.Contains(config, unwanted) {
			t.Fatalf("expected repo Codex config to omit %q, got %q", unwanted, config)
		}
	}
