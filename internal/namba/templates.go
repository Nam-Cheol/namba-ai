...
func renderAgents(profile initProfile) string {
	return fmt.Sprintf("# NambaAI\n\n"+
		...
		"- When delegating work with Codex multi-agent features, use custom agents under `.codex/agents/*.toml`.\n\n"+
		"## Workflow\n\n"+
...
func renderCodexUsage(profile initProfile) string {
	lines := []string{
		...
		"- Creates Codex custom agents under `.codex/agents/*.toml`.",
		...
		"4. Use built-in Codex delegation with custom agents in `.codex/agents/*.toml` when multi-agent work is appropriate.",
		...
		"- Claude subagents become explicit `.toml` custom agents used with Codex multi-agent delegation.",
...
func renderPlannerCustomAgent() string {
	lines := []string{
		"name = \"namba-planner\"",
		"description = \"Break down a SPEC package into an execution plan without editing files.\"",
		"",
		"prompt = \"You are Namba Planner.\n\nRead spec.md, plan.md, and acceptance.md for the target SPEC.\nIdentify target files, key risks, and validation commands.\nProduce a concise implementation plan for the main session.\nDo not edit files directly.\"",
	}
	return strings.Join(lines, "\n") + "\n"
}
...
func renderImplementerCustomAgent() string {
	lines := []string{
		...
		"description = \"Implement approved SPEC tasks and report validation status.\"",
		"",
		"prompt = \"You are Namba Implementer.\n\nImplement only the files assigned by the main session.\nPreserve methodology rules from .namba/config/sections/quality.yaml.\nAfter changes, report validation status and residual risk.\"",
	}
	return strings.Join(lines, "\n") + "\n"
}
...
func renderReviewerCustomAgent() string {
	lines := []string{
		"name = \"namba-reviewer\"",
		"description = \"Review SPEC implementation quality and acceptance coverage before sync.\"",
		"",
		"prompt = \"You are Namba Reviewer.\n\nCompare implementation outcomes with acceptance.md.\nVerify validation output and expected artifacts.\nCall out regressions, missing tests, and documentation drift.\nDo not rewrite implementation unless explicitly asked.\"",
	}
	return strings.Join(lines, "\n") + "\n"
}
