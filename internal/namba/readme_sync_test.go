...
func TestRenderManagedProjectWorkflowGuidePlanAndFixSectionPreservesLocalizedAnchors(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			planNeedle: "- `namba plan \"description\"`: create the next feature SPEC package and review artifacts, then continue with `$namba-plan-review` unless `--no-review` is present.",
		...
		},
		{
			...
			planNeedle: "- `namba plan \"description\"`: 다음 기능 SPEC 패키지와 review artifact를 만들고, `--no-review`가 없으면 `$namba-plan-review`로 바로 이어갑니다.",
		...
		},
		{
			...
			leadNeedle: "`$namba-help`",
			planNeedle: "- `namba plan \"description\"`: 次の機能 SPEC パッケージと review artifact を作成し、`--no-review` がなければ `$namba-plan-review` に続けます。",
		...
		},
		{
			...
			leadNeedle: "`$namba-help`",
			planNeedle: "- `namba plan \"description\"`: 创建下一个功能 SPEC 包和 review artifact；如果没有 `--no-review`，会继续交给 `$namba-plan-review`。",
