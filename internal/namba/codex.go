...
const (
	...
	repoCodexHooksPath  = ".codex/hooks.json"
	repoCodexHooksDir   = ".codex/hooks"
...
func codexSkillTemplates(profile initProfile) map[string]string {
	return map[string]string{
		...
		filepath.ToSlash(filepath.Join("namba-review-resolve", "SKILL.md")):     renderReviewResolveCommandSkill(),
		filepath.ToSlash(filepath.Join("namba-run", "SKILL.md")):                renderRunCommandSkill(profile),
		filepath.ToSlash(filepath.Join("namba-queue", "SKILL.md")):              renderQueueCommandSkill(),
		...
		filepath.ToSlash(filepath.Join("namba-release", "SKILL.md")):            renderReleaseCommandSkill(),
...
func isManagedRepoCodexHookPath(rel string) bool {
	switch rel {
	case repoCodexHooksPath:
		return true
...
func codexNativeIssues(root string) []string {
	checks := []struct {
		...
		{label: ".agents/skills/namba-queue/SKILL.md", path: filepath.Join(root, ".agents", "skills", "namba-queue", "SKILL.md")},
