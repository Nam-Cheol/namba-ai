...
| `.claude/agents/*.md` | `.codex/agents/*.toml` custom agent (`.md` mirror) |
...
- `namba update`: regenerate `AGENTS.md`, `.agents/skills/`, `.codex/skills/`, `.codex/agents/*.toml` custom agents (plus `.md` mirrors), and `.codex/config.toml` from `.namba/config/sections/*.yaml`.
...
대신 Codex에서 사용할 **custom agent TOML**을 생성하고, 읽기 쉬운 role card(.md)도 함께 생성합니다.
